...
@register("ocr-mlt-50m", "interfaze-ai OCR-MLT-50M: 50.2M pairs, 50 languages")
def convert_ocr_mlt_50m(out_dir):
    """Convert OCR-MLT-50M HuggingFace dataset to shards."""
    from datasets import load_dataset
    from PIL import Image as PILImage

    mlt_dir = DATA_DIR / "ocr-mlt-50m"

    # Map dataset language codes to our script names
    lang_to_script = {
        "en": "latin", "de": "latin", "fr": "latin", "es": "latin",
        "pt": "latin", "it": "latin", "nl": "latin", "pl": "latin",
        "tr": "latin", "sv": "latin", "cs": "latin", "ro": "latin",
        "da": "latin", "fi": "latin", "hu": "latin", "hr": "latin",
        "sk": "latin", "sl": "latin", "lt": "latin", "lv": "latin",
        "et": "latin", "mt": "latin", "ga": "latin", "ms": "latin",
        "id": "latin", "tl": "latin", "sw": "latin", "vi": "latin",
        ...
        "ko": "korean",
        "am": "ethiopic",
        "zh": "han_kana", "ja": "han_kana",
    }

    # Load from local parquet files or stream from HF
    try:
        ds = load_dataset(str(mlt_dir), split="train")
    except Exception:
        print("  Local load failed, streaming from HuggingFace...")
        ds = load_dataset("interfaze-ai/ocr-mlt-50m", split="train", streaming=True)

    # Group by script
    script_data: dict[str, tuple[list, list]] = {}
    n_processed = 0
    n_skipped = 0
    ...
    for sample in ds:
        ...
        if script not in script_data:
            script_data[script] = ([], [])
        script_data[script][0].append(tensor)
        script_data[script][1].append(text)

        n_processed += 1
        if n_processed % 100000 == 0:
            print(f"    {n_processed:,} processed, {n_skipped:,} skipped, "
                  f"{len(script_data)} scripts", flush=True)

        # Save in batches to avoid memory issues
        for s, (imgs, lbls) in list(script_data.items()):
            if len(imgs) >= SHARD_SIZE:
                saved = save_shards(imgs[:SHARD_SIZE], lbls[:SHARD_SIZE],
                                    s, out_dir, prefix="mlt50m")
                script_data[s] = (imgs[SHARD_SIZE:], lbls[SHARD_SIZE:])
                print(f"    Saved {saved} {s} images", flush=True)

    # Save remaining
    for s, (imgs, lbls) in script_data.items():
        if imgs:
            saved = save_shards(imgs, lbls, s, out_dir, prefix="mlt50m")
            print(f"    Saved {saved} {s} images", flush=True)

    print(f"  Total: {n_processed:,} images, {n_skipped:,} skipped")


def process_image_pil(img, height=32, max_width=768):
    """Process a PIL image (already loaded) to model input format."""
    if img.width < 4 or img.height < 4:
        return None
