...
def get_han_codec() -> CJKCodec:
    """Build the Han codec from the shared CJK tables.

    Hard-fails when the vocab table is missing: a silently vocab-less codec
    corrupts every Han target (same failure class as the missing-raqm bug),
    so absence must stop the run, not degrade it.
    """
    global _han_codec
    if _han_codec is not None:
        return _han_codec

    vocab_path = _FUSIONS_DIR / "cjk_vocab.txt"
    if not vocab_path.exists():
        raise RuntimeError(
            f"Han vocab table missing: {vocab_path}. The codec cannot be "
            "built without it — check the training_data assets.")
    ...
    if not mapping_path.exists():
        raise RuntimeError(
            f"Han visual mapping missing: {mapping_path}. Rare-char ALT "
            "pairs cannot be built without it — check the training_data "
            "assets.")
    alt_mapping = {}
    ...
    _han_codec = CJKCodec(_HAN_BASE, freq_chars, alt_mapping)
    return _han_codec
